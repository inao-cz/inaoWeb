...
class UserController extends AbstractController
{
    ...
    {
        //$history = $this->getDoctrine()->getRepository(Log::class)->findBy();
        $parsedHistory = [];
        /** @var Log $item */
...
//        }
    ...
    public function inviteUserAction(Request $request, EndpointUtil $endpointUtil, MailUtil $mailUtil): Response
    {
        $invite = new Invite();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $manager->flush();
            $mailUtil->sendEmail('register', 'Invited to inao.xn--6frz82g', $invite->getEmail(), $this->generateUrl('security-register', ['invite' => $invite->getCode()]));
        }
    ...
    #[Route(path: '/redirect/create', name: 'redirect-create'), IsGranted("ROLE_LINKS")]
    public function createNewRedirect()
    {
        $links = new Links();
        $form = $this->createForm(RedirectType::class, $links);
        return $this->render('user/redirect.html.twig', ['redirectForm' => $form->createView()]);
    }
